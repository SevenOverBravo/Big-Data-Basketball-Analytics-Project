...
If this project were to be repeated, the classification system would experience a major overhaul. One potential approach is use a metric independent from any of the efficiency statistics utilized in the study. Since each regression model had differing dependent variables depending on which efficiency metric they predicted, comparison between models with different metrics became a massive problem, as each metric possessed its own means of calculation, scale, and distribution. Having a consistent dependent variable for each regression would not only remedy the comparison issue, but also contribute towards solving the previously mentioned problems with the season classification system.

After researching multiple advanced statistics, the best option for a consistent dependent variable would be [BBall Index's](https://www.bball-index.com/lebron-introduction/) LEBRON (Luck-adjusted player Estimate using a Box prior Regularized ON-off) metric. LEBRON, according to BBall Index's website, evaluates a player's offensive and defensive impact per 100 on-court possessions through consideration of weighted box score, on-off calculations, teammate/opponent quality, luck adjustments (factors due to luck/variance rather than player impact), and Wins Added (number of wins a player has added to a team in a given season). Given the comprehensive level of relevant factors included in its calculations and adjustments that account for random variance, LEBRON is among the most complete estimates of total player impact available. Additionally, LEBRON data goes back to 2009, leaving a nearly identical of seasons for prediction, and data can be freely downloaded from BBall Index's website. More details about LEBRON can be found using the above link. 

Instead of categorizing seasons based on the mean and standard deviation of each value, bounds for each classification will be a range of LEBRON differential values chosen based on research of career breakouts (leap from a starter to all-star, for example) and declines (downgrade from a starter to rotation player or traded away) as they relate to such values. As a result, the first mentioned problem with classification would be greatly remedied, since it's no longer assumed that the number of breakout seasons stays the same across each season. This is because using a individual value instead of a distribution to categorize means that some seasons, where LEBRON scores are unextraordinary, will have only one or two breakout seasons, whilst others may have several. Such a measure also helps the second issue, as the proportion of players that fall into each category will shift every season. Because of this, biasing estimates toward the middle categories will become a lot less effective in improving accuracy, and models will have to prove they're worth their salt legitimately.
